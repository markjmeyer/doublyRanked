...
#### save output ####
fname <- paste('/Users/mjm556/Dropbox/Research/Drafts/Wilcoxon FDA/Simulations/Gaussian/pnorm/KW/N1_', n1, sep = '')
setwd(fname)
filename <- paste('kw_pnorm_g_', n1, '_r_fpca.RData', sep = '')
...
for(i in 1:length(rho)){
  for(k in 1:length(S)){
    ...
    if(plotFlag){
      if(one){
        figname   <- paste('kw_phi_n_', N, '_S', S[k], '_r', round(100*rho[i]),  '_g.pdf', sep = '')
      } else{
