...
#### save output ####
fname <- paste('~/Dropbox/Research/Drafts/Wilcoxon FDA/Simulations/Gaussian/gamma/MWW/N1_', n1, sep = '')
setwd(fname)
filename <- paste('mww_gamma_g_', n1, '_r_fpca.RData', sep = '')
...
for(i in 1:length(rho)){
  for(k in 1:length(S)){
    ...
    if(plotFlag){
      figname   <- paste('mww_gam_n1_', n1,'_n2_', n2, '_S', S[k], '_r', round(100*rho[i]),  '_g_fpca.pdf', sep = '')
      pdf(figname)
