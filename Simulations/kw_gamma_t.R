...
#### save output ####
fname <- paste('/Users/mjm556/Dropbox/Research/Drafts/Wilcoxon FDA/Simulations/MVT/gamma/KW/N1_', n1, sep = '')
setwd(fname)
filename <- paste('kw_gamma_t_', n1, '_r_fpca.RData', sep = '')
...
for(i in 1:length(rho)){
  for(k in 1:length(S)){
    ...
    if(plotFlag){
      # if(one){
      #   figname   <- paste('kw_gam_n_', N, '_S', S[k], '_r', round(100*rho[i]),  '_t.pdf', sep = '')
      # } else{
      #   figname   <- paste('kw_gam_n_', N, '_S', S[k], '_r', round(100*rho[i]),  '_c_2.pdf', sep = '')
      # }
      # pdf(figname)
      plot(1-sc, apply(pvals[i, k, , , 1] < 0.05, 1, mean), type = 'n', ylim = c(0,1), main = paste('Rho = ', rho[i], sep = ''), ylab = '',
