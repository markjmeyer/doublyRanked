...
#### save output ####
fname <- paste('/Users/mjm556/Dropbox/Research/Drafts/Wilcoxon FDA/Simulations/LogNormal/pnorm/MWW/N1_', n1, sep = '')
setwd(fname)
filename <- paste('mww_pnorm_l_', n1, '_r_fpca.RData', sep = '')
...
for(i in 1:length(rho)){
  for(k in 1:length(S)){
    ...
    if(plotFlag){
      figname   <- paste('mww_phi_n1_', n1,'_n2_', n2, '_S', S[k], '_r', round(100*rho[i]),  '_l_fpca.pdf', sep = '')
      pdf(figname)
