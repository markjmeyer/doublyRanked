...
#### specifications ####
# sn    <- c(1, 1.25, 1.5)
n1    <- n2 <- 10 # 10, 25, 50 -> 20, 50, 100 or 6, 12, 18 -> 12, 24, 36
N     <- n1 + n2
S     <- c(40, 80, 160, 320)
...
#### save output ####
fname <- paste('~/Dropbox/Research/Drafts/Wilcoxon FDA/Simulations/MVT/gamma/MWW/N1_', n1, sep = '')
setwd(fname)
filename <- paste('mww_gamma_t_', n1, '_r_fpca.RData', sep = '')
...
for(i in 1:length(rho)){
  for(k in 1:length(S)){
    ...
    if(plotFlag){
      figname   <- paste('mww_gam_n1_', n1,'_n2_', n2, '_S', S[k], '_r', round(100*rho[i]),  '_t_fpca.pdf', sep = '')
      pdf(figname)
